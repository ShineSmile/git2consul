...
exports.clone = function(repo_url, branch_name, cwd, cb) {
  run_command('git -c http.extraheader="AUTHORIZATION: basic ' + process.env.AZURE_REPO_AUTH + '" ' + 'clone -b ' + branch_name + ' ' + repo_url + ' ' + branch_name, cwd, cb);
};

exports.pull = function(branch_name, cwd, cb) {
  run_command('git -c http.extraheader="AUTHORIZATION: basic ' + process.env.AZURE_REPO_AUTH + '" ' + 'pull origin' + ' ' + branch_name, cwd, cb);
};
